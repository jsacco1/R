#limma tutorial
#from: RNA-seq analysis is easy as 1-2-3 with limma, Glimma and edgeR (https://www.bioconductor.org/help/workflows/RNAseq123/#introduction)
...
#from https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE63310&format=file, 
#and extract the relevant files from this archive. The code below will do this, 
#or you can do this step manually and then move on.
...
#Whilst each of the nine text files can be read into R separately 
#and combined into a matrix of counts, edgeR offers a convenient way 
#to do this in one step using the readDGE function. 
x <- readDGE(files, columns=c(1,3))
class(x)
#The resulting DGEList-object contains a matrix of counts with 27,179 rows associated with unique Entrez gene 
#identifiers (IDs) and nine columns associated with the individual samples in the experiment.
dim(x)
...
#This should include experimental variables, both biological and technical, 
#that could have an effect on expression levels. 
#Examples include cell type (basal, LP and ML in this experiment), genotype (wild-type, knock-out), 
#phenotype (disease status, sex, age), sample treatment (drug, control) and batch information (date experiment was performed if samples were collected and analysed at distinct time points) to name just a few.
